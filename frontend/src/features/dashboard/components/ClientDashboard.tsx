...
import { Dumbbell, Calendar, User, BookOpen, ChevronRight } from 'lucide-react'
...
export function ClientDashboard() {
    ...
    const { data: memberships } = useSuspenseQuery(membershipsQueryOptions({ status: 'Active' }))
    const { data: programs } = useSuspenseQuery(programsQueryOptions({ status: 'IN_PROGRESS' }))

    const activeMembership = memberships.results[0] ?? null
    const activeProgram = programs.results[0] ?? null

    // Get workouts for the active program's next phase if available
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
                ...
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-grey-900 border border-grey-800">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-sm font-medium text-grey-100 truncate">
                            {activeMembership ? activeMembership.trainer_name : 'No trainer yet'}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-grey-900 border border-grey-800">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-sm font-medium text-grey-100 truncate">
                            {activeProgram ? activeProgram.program_name : 'No active program'}
                        </p>
                    </div>
                </div>
            </div>
