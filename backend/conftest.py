...
@pytest.fixture
def completed_phase(active_membership):
    now = timezone.now()
    today = timezone.localdate()
    ...
    program = baker.make(
        ...
        experience_level=experience_level,
        status=ProgramStatusOption.objects.get(
            code=ProgramStatusesVocabulary.COMPLETED
        ),
        created_by_trainer=active_membership.trainer.user,
        last_edited_by=active_membership.trainer.user,
        submitted_for_review_at=now,
    ...
    phase_option, _ = ProgramPhaseOption.objects.get_or_create(
        code="HYPERTROPHY",
        defaults={"label": "Hypertrophy", "default_duration_days": 28},
    )
    return baker.make(
        "programs.ProgramPhase",
        program=program,
        phase_option=phase_option,
        status=ProgramPhaseStatusOption.objects.get(
            code=ProgramPhaseStatusesVocabulary.COMPLETED
        ),
        phase_goal="Build size",
        sequence_order=1,
        planned_start_date=today,
        planned_end_date=today + timezone.timedelta(days=28),
        actual_start_date=today,
        actual_end_date=today + timezone.timedelta(days=28),
        started_at=now,
        completed_at=now,
        created_by_trainer=active_membership.trainer.user,
        last_edited_by=active_membership.trainer.user,
