...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        cache_file_path = os.path.join(curr_dir, "enrichment_data.json")
        ...
        try:
            with open(file_path) as f:
                ...
                for exercise_data in data.get("exercises", []):
                    ...
                    # Should fail if theres no API Key
                    if not api_key:
                    ...
                    if api_name not in api_cache:
                        ...
                        # Prevent flooding of API calls to API Ninja
                        time.sleep(2)
                        ...
                        # Loop over results for visiblity
                        ...
                        if len(data_arr) == 0:
                            raise ValueError(
                                f"API Response should have returned at least 1 result.\
                                    Returned: {len(data_arr)}"
                            )

                        # Get the enrichment data from the array (Take first for MVP)
